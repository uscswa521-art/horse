...
> **想要「入場、有座位、有臨場感」嗰種？** 用同一份片單仲有另一個空間 —— **一號影廳 `theatre.html`**：
> 撕飛入場 → 推門行入影廳 → 揀個位坐低 → 場燈熄、絲絨幕拉開 → 條片就喺銀幕上面播。
> 詳細睇下面 [一號影廳](#-一號影廳-theatrehtml)。`python3 serve.py` 預設就係開呢個。
...
唔係遊戲、冇小地圖、冇準星 —— 係一間戲院。你係觀眾，唔係角色。

```
theatre.html        ← 入場券 / 節目表 / 現正放映 (HUD 全部係紙同銅)
theatre.js          ← 影廳引擎 (WebGL2, 零依賴): 門廳、雙門、六行座位、絲絨幕、壁燈、放映光束
theatre-tex.js      ← 程序生成貼圖: 牆肋、地毯、絲絨、幕楣、座位銅牌、招牌、海報
theatre-audio.js    ← 環境聲 (Web Audio 合成, 冇音檔): 門聲、腳步、開場鐘、幕機、觀眾窸窣
data/videos.json    ← 同影片大堂共用嘅片單
```
...
| 幕 | 發生咩事 |
|---|---|
| **入場券** | 一張紙飛：今場放映邊套、場次、票號。撳「入場」就撕飛。（上次坐過嘅位會記住，可以「直接入座」） |
| **推門** | 6.6 秒由門廳行入影廳：雙門推開、腳步聲、鏡頭抬頭見到成間廳。`跳過` 或者撳任何掣即刻去揀位 |
| **揀位** | A–F 六行 60 個位，銅牌寫住座號。有人坐嘅位係黑影。滑鼠掃過會亮牌，撳一下就坐；`B` / 「幫我揀個好位」= D6 |
| **坐低** | 鏡頭滑去嗰行再坐低，場燈慢慢暗到 45%，票根蓋上座號 |
| **開場** | 場燈熄到 12%、幕機聲、絲絨幕 2.8 秒拉開、銀幕亮起，條片就喺幕後面（真 YouTube iframe，位置同 3D 銀幕對齊） |
| **散場** | 播完幕會合埋，節目表自動彈出叫你揀「下一場」 |

### 操作

| 操作 | 做咩 |
|---|---|
| **滑鼠拖** | 望嚟望去（坐低之後望正銀幕會自動貼平，畫面唔會歪） |
| `←` `→` `↑` `↓` | 揀位嗰陣揀隔籬位；坐低之後 `E` 或者「起身」 |
| `Enter` / `B` | 坐落而家揀住 / 最好嘅位 |
| `P` | 節目表（直播中 / 熱門 / 全部），撳一套就換片：幕合埋 → 換 → 再拉開 |
| `1`–`9` | 節目表入面直接跳去第幾套 |
| `M` | 靜音 |
| `Esc` | 閂節目表 |
| 手機 | 拖住望；撳座位坐；底下有 `◀位` `位▶` 換位 |
...
|---|---|
| `?v=<video id>` | 指定今場放映邊套 |
| `?dolly=0` | 唔要推門嗰段，直接去揀位（`prefers-reduced-motion` 開咗都會自動咁樣） |
| `?q=lo` | 低畫質（座位少啲部件、冇塵、冇顆粒）；跑得慢會自動降 |
| `?crowd=<n>` | 硬性指定坐咗幾多人（預設按今場觀看人數推算） |
| `?data=<url>` | 換一份片單 |

`file://` 直接 double-click 都開到，會用內建示範片單（畫面右上角會寫明）。要真片單就 `python3 serve.py`。
